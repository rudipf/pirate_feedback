...
function util.import_members(file)
 ...
 if config.delete_private_data_after then
	...
	local locked=db:query("select m.id,max(until) from member m join member_history mh on m.id=mh.member_id where mh.active='t' and (m.locked='t' or m.locked_import='t') group by m.id having(max(until))+Interval '"..config.delete_private_data_after.."'" <current_date , "list")

	for i, locked_member in ipairs(locked) do
		db:query("SELECT delete_member("..locked_member[1]..")")
         	print("member_id "..locked_member[1].." deleted private data" )
		db:query("SELECT delete_member(" ..locked_member[1].. ")")
